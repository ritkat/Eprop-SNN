    ...
    for trial in range(1,args.trials+1):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if not args.skip_test:
                ...
                accuracy_ep, loss_ep=do_epoch(args, False, model, device, test_loader, optimizer, loss, 'test')
            accuracy_epoch.append(accuracy_ep)
...
def do_epoch(args, do_training, model, device, loader, optimizer, loss_fct, benchType):
    accuracy_ep=[] #List to store the accuracy per epoch
    loss_ep=[]
    ...
    if args.classif:
        ...
        accuracy_ep.append(score.item()/length*100)
        loss_ep.append(loss.item())
    else:
        print("\t\t Loss "+info+str(loss.item()))

    return accuracy_ep, loss_ep
